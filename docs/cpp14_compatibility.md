...
### 1. C++17ユーザー定義リテラル → 完全削除

**問題**: C++17の`template <typename T, T... Chars>`を使用したユーザー定義リテラルはC++14では使用不可。

**削除内容**: `/workspace/include/omusubi/core/static_string.hpp`
...
- C++17対応マイコンを使用しないため、条件付きコンパイル（`#if __cplusplus >= 201703L`）も不要
- プロジェクト全体をC++14に統一するため完全削除
...
### 2. `inline constexpr` 関数 → `constexpr` 関数

**問題**: C++17では`inline constexpr`が許可されていますが、C++14では`constexpr`関数は暗黙的に`inline`。
...
**理由**: C++14では`constexpr`関数は暗黙的に`inline`なので、明示的な`inline`キーワードは冗長

### 3. `inline constexpr` 変数 → `constexpr` 変数
...
**理由**: C++14では名前空間スコープの`constexpr`変数は各翻訳単位で内部リンケージを持つため、`inline`は不要

### 4. Makefile の修正

**修正内容**: `/workspace/Makefile` および `/workspace/test/Makefile`
...
5. `/workspace/test/Makefile` - C++14フラグ設定
6. 各ドキュメント - C++14準拠の説明を追加
