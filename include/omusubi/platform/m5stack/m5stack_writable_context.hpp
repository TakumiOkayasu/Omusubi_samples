...
public:
    M5StackWritableContext(SerialContext* serial0, SerialContext* serial1, SerialContext* serial2, BluetoothContext* bluetooth)
        : serials_ {serial0, serial1, serial2}, bluetooth_(bluetooth) {}

    ~M5StackWritableContext() override = default;

    SerialContext* get_serial_context(uint8_t port) const override { return (port < 3) ? serials_[port] : nullptr; }
